...
// Model for Course entity
    models.Course = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/JavaEE7Thesis/webresources/vasouv.javaee7thesis.courses.course/",
        idAttribute: 'idcourse',
        defaults: {
            image: "",
            price: "",
            description: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idcourse');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/JavaEE7Thesis/webresources/vasouv.javaee7thesis.courses.course/';
            }
    ...
    // Collection class for Course entities
    models.CourseCollection = Backbone.Collection.extend({
        model: models.Course,
        url: "http://localhost:8080/JavaEE7Thesis/webresources/vasouv.javaee7thesis.courses.course/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CourseCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-course-list-itemis template identifier for item
                        templateName: '#tpl-course-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.course = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.course,
                    // tpl-course-details is template identifier for chosen model element
                    templateName: '#tpl-course-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Course();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-course-details is a template identifier for chosen model element
                templateName: '#tpl-course-details',
